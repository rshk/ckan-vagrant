class ckan {
  ...
  file {'/etc/init.d/jetty':
    ...
    mode   => 0755,
    source => 'puppet:///modules/ckan/jetty.sh',
  }
  file {'/etc/default/jetty':
    ensure => file,
    content => template('ckan/jetty_config.erb'),
  }
  file {$solr_logs:
    require => User['solr'],
    ensure  => directory,
  ...
  # ---------
  # Webserver
  # todo: we want to use nginx!
  # ---------

  class { 'nginx': }
  class { 'mcollective': }
  nginx::resource::upstream { 'ckan_app':
    ensure => present,
    members => [
      'localhost:5000'
    ],
  ...
  file { "/home/vagrant/.noserc":
    ...
    source => "puppet:///modules/ckan/noserc_template",
    mode   => 644,
  }
}
